...
```mermaid
graph TD
    A[Client User Interface React.js] -->|HTTP/REST| B[API Gateway Node.js/Express]
    B --> C{Auth Middleware JWT}
    C -->|Validated| D[Core Services]
    D --> E[Assignment Service]
    D --> F[Mentorship & Dashboard Service]
    
    E --> G{Evaluation Router}
    G -->|Programming| H[Code Execution Environment Sandboxed]
    G -->|Theory/Handwritten| I[OCR Engine & AI Prompt Builder]
    
    I --> J[External LLM / Vision APIs]
    
    H --> K[Database Layer MongoDB]
    I --> K
    F --> K
...
stateDiagram-v2
    [*] --> Login
    Login --> Dashboard
    Dashboard --> ViewPendingAssignments
    ViewPendingAssignments --> SelectAssignment
    
    state SelectAssignment {
        direction LR
        CheckType --> Programming : If Code
        CheckType --> Theory : If Document
        
        Programming --> WriteCodeIDE
        WriteCodeIDE --> RunAgainstSample
        RunAgainstSample --> FinalSubmit
        
        Theory --> TypeAnswerOrUploadPDF
        TypeAnswerOrUploadPDF --> FinalSubmit
    }
    
    SelectAssignment --> AwaitEvaluation
    AwaitEvaluation --> ViewGradeAndFeedback
    ViewGradeAndFeedback --> [*]
```
...
stateDiagram-v2
    ...
    FacultyDashboard --> CreateAssignment
    FacultyDashboard --> MonitorSubmissions
    
    CreateAssignment --> DefineParameters
    DefineParameters --> SetRubricOrTestCases
    SetRubricOrTestCases --> PublishToStudents
    PublishToStudents --> [*]
    
    MonitorSubmissions --> ReviewAIGrades
    ReviewAIGrades --> ApproveGrade
    ReviewAIGrades --> OverrideGradeOption
    OverrideGradeOption --> UpdateFinalGrade
    ApproveGrade --> PublishResults
    UpdateFinalGrade --> PublishResults
    PublishResults --> [*]
```
...
stateDiagram-v2
    ...
    AdminDashboard --> ReviewSystemHealth
    AdminDashboard --> ManageUsers
    AdminDashboard --> AuditLogs
    
    ManageUsers --> BulkUploadCSV
    ManageUsers --> ModifyRBACPolicies
    
    AuditLogs --> ReviewSecurityEvents
    AuditLogs --> InvestigateOverrides
...
sequenceDiagram
    participant Student
    participant ReactUI as Web Interface
    participant NodeAPI as Backend Server
    participant DB as MongoDB
    participant Runner as Code/AI Executor

    Student->>ReactUI: Submits Code Assignment
    ReactUI->>NodeAPI: POST /api/submit (Token, Payload)
    NodeAPI->>NodeAPI: Validate JWT Token
    NodeAPI->>DB: Log Submission Attempt
    NodeAPI->>Runner: Initialize compilation (C/Java/Python)
    Runner-->>Runner: Execute against Test Case 1..N
    Runner-->>NodeAPI: Return Results (Pass/Fail matrix)
    NodeAPI->>NodeAPI: Calculate Final Score vs Max Marks
    NodeAPI->>DB: Update Submission with Grade & Status
    NodeAPI-->>ReactUI: Return 200 OK (Evaluation Complete)
    ReactUI-->>Student: Display Score & Test Case details
```
...
graph LR
    ...
    subgraph Level 0 System Core
        SYS{Advanced Academic\nEvaluation System}
    end
    
    S -->|Submissions, Code, PDFs| SYS
    SYS -->|Grades, Feedback, Notes| S
    
    F -->|Rubrics, TestCases, Prompts| SYS
    SYS -->|Automated Results, Analytics| F
    
    A -->|System Config, User Lists| SYS
    SYS -->|Audit Logs, Global Health| A
```
...
```mermaid
graph TD
    A[Raw DB Grades] --> B[Mongo Aggregation Pipeline]
    B --> C{Data Separation}
    C -->|Student View| D[Personal Progress Curve]
    C -->|Faculty View| E[Class Average & Metrics]
    C -->|HOD View| F[Department Wise Comparisons]
    
    B --> G[Risk Assessment Algorithm]
    G -->|If Score < 40% over 3 tests| H[Flag as At-Risk]
    H --> I[Notify Mentor]
```
...
flowchart TD
    %% Define Nodes
    A_START(Faculty Logs In) --> B_CREATE[Faculty Creates Assignment]
    B_CREATE --> C_PARAMS{Is Programming?}
    
    C_PARAMS -->|YES| D_PROG[Enter Test Cases & Constraints]
    C_PARAMS -->|NO| D_THEO[Enter Rubric Key & Expected Format]
    
    D_PROG --> E_PUB[Publish to Student Portal]
    D_THEO --> E_PUB
    
    E_PUB --> F_NOTIFY[[System Notifies Students]]
    F_NOTIFY --> G_STUDENT(Student Accesses Assignment)
    
    G_STUDENT --> H_TYPE{Submission Mode}
    H_TYPE -->|Code| I_CODE[Web IDE: Write Code]
    H_TYPE -->|Document| I_DOC[Upload PDF/Image]
    H_TYPE -->|Text| I_TEXT[Rich Text Editor]
    
    I_CODE --> J_SUBMIT[Submit to Backend]
    I_DOC --> J_SUBMIT
    I_TEXT --> J_SUBMIT
    
    J_SUBMIT --> K_EVAL{Evaluation Routing}
    
    K_EVAL -->|Code Runner Module| L_EXECUTE[Compile & Execute against Tests]
    K_EVAL -->|AI Evaluator Module| L_OCR[OCR Preprocessing]
    
    L_OCR --> L_AI[Submit Student Text + Rubric to LLM]
    
    L_EXECUTE --> M_GRADE[Calculate Final Score]
    L_AI --> M_GRADE
    
    M_GRADE --> N_DB[(Save to Database)]
    N_DB --> O_DASH_S[Update Student Dashboard]
    N_DB --> O_DASH_F[Update Faculty/HOD Analytics]
    
    O_DASH_S --> P_END_S(Student Reviews Feedback)
    O_DASH_F --> P_END_F(System Checks At-Risk Thresholds)

    style A_START fill:#2ecc71,stroke:#333,stroke-width:2px;
    style P_END_S fill:#3498db,stroke:#333,stroke-width:2px;
    style P_END_F fill:#e74c3c,stroke:#333,stroke-width:2px,color:#fff;
    style N_DB fill:#f1c40f,stroke:#333,stroke-width:2px;
```
