...
const QuizBuilder: React.FC<Props> = ({ formData, setFormData }) => {
    ...
    const addQuestion = (type: string) => {
        ...
        if (type === 'MCQ' || type === 'MultipleCorrect') {
            newQ.options = ['', '', '', ''];
            // Single answer for MCQ, Array for MultipleCorrect
    ...
    return (
        <div className="space-y-6">
            <div className="bg-purple-50/50 p-4 border border-purple-100 rounded-xl">
                ...
                <p className="text-sm text-purple-700">Configure an online timed test. Students will take this quiz directly on the platform and it will be auto-graded based on your chosen answers.</p>
            </div>
            ...
            <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                    ...
                    <button type="button" onClick={() => addQuestion('MultipleCorrect')} className="bg-purple-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-purple-700 transition">+ Add Multiple Checkboxes</button>
                </div>
            </div>

            {marksMismatch && (
                <div className="p-3 bg-red-50 text-red-700 text-sm font-bold rounded-lg border border-red-100 flex items-center gap-2">
                    <span>⚠️ Error: The sum of all question marks ({totalQuizMarks}) must exactly match the Assignment Total Marks ({formData.maxMarks || 0}).</span>
                </div>
            )}
            ...
            <div className="space-y-4">
                    ...
                    <div key={i} className={`p-5 border ${q.questionType === 'True/False' ? 'border-teal-200 bg-teal-50/10' : q.questionType === 'MCQ' ? 'border-indigo-200 bg-indigo-50/10' : 'border-purple-200 bg-purple-50/10'} rounded-xl relative shadow-sm`}>
                        ...
                        {(q.questionType === 'MCQ' || q.questionType === 'MultipleCorrect') && (
                            <div className="space-y-2 mt-4 pl-4 border-l-2 border-indigo-200">
                                ...
                                <button type="button" onClick={() => updateQuestion(i, 'options', [...q.options, ''])} className="text-xs font-bold text-indigo-600 mt-2">+ Add Option</button>
                            </div>
                        )}
                        ...
                        {q.questionType === 'True/False' && (
                            <div className="flex items-center gap-6 mt-4 pl-4 border-l-2 border-teal-200">
                                ...
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" checked={q.correctAnswer === 'True'} onChange={() => updateQuestion(i, 'correctAnswer', 'True')} className="w-4 h-4 text-teal-600" />
                                    <span className="text-sm font-bold">True</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" checked={q.correctAnswer === 'False'} onChange={() => updateQuestion(i, 'correctAnswer', 'False')} className="w-4 h-4 text-teal-600" />
                ...
                {questions.length === 0 && (
                    <div className="text-center py-8 bg-gray-50 rounded-xl border border-gray-200 border-dashed">
                        <p className="text-gray-500 text-sm">No quiz questions added yet. Use the buttons above to build your test.</p>
                    </div>
                )}
            </div>
            ...
            <div className="flex justify-end pt-4">
                <div className={`px-4 py-2 rounded-lg font-bold text-sm ${marksMismatch ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                    Quiz Total Marks: {totalQuizMarks} / {formData.maxMarks || 0}
                </div>
            </div>
        </div>
    );
